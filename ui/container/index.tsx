...
export const DivPublicar = ({ children }: { children: React.ReactNode }) => (
  <div
    className={` bg-primary  w-full h-max  p-4 max-md:p-2 pb-2 flex flex-col justify-evenly gap-4 rounded-md shadow-container  max-md:w-auto  dark:bg-darkComponet dark:text-primary dark:shadow-none text-center`}>
    {children}
  </div>
);
...
export const DivAllAmistades = ({
  ...
  <div
    className={`bg-primary rounded-md shadow-container dark:shadow-dark max-md:mt-4 w-[185px]  dark:bg-darkComponet grid ${
      !requestClassDuo ? 'grid-rows-[repeat(1,200px_100px)]' : 'grid-rows-[repeat(1,200px_150px)]'
    }`}>
    {children}
  </div>
);
