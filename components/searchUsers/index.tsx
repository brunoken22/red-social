...
export function SearchUser() {
  ...
  return <div className='relative'>{query ? hits.length ? <Hits hitComponent={Hit} className='dark:text-primary text-secundary  bg-primary dark:bg-dark p-2' /> : <p className='absolute bg-[#ff4a3d] p-4'>No se encontraron resultado</p> : null}</div>;
}
type HitProps = {
  hit: Hit;
};
function Hit({ hit }: HitProps) {
  ...
  return (
    <Link className='w-full hover:opacity-70' href={'/amigos/' + hit.objectID}>
      <DivLinkUser>
        <FotoPerfil className='w-[40px] h-[40px]' img={hit.img} connect={dataIsConnect?.find((eConnect: any) => hit.id == eConnect.id)?.connect && true}></FotoPerfil>
